...
Result_save_path = namedtuple('result_save_path',
                              'encoder_train_pre, encoder_test_pre, decoder_train_pre, decoder_test_pre, train_bits, test_bits')
Model_save_path = namedtuple('model_save_path', 'encoder_save_path, decoder_save_path, mapping_model_save_path')
...
def model_load(model_path: Model_save_path):
    mapping_model = tf.saved_model.load(model_path.mapping_model_save_path)
    encoder_model = tf.saved_model.load(model_path.encoder_save_path)
    decoder = tf.saved_model.load(model_path.decoder_save_path)
    return mapping_model, encoder_model, decoder


class Saver:
    @classmethod
    def save_model(cls, encoder, decoder, mapping_model, x_train, x_test,
                   model_save_path: Model_save_path, result_save_path:Result_save_path):
        encoder_train_pre = encoder(x_train)  # 经过信道加完噪声
        encoder_test_pre = encoder(x_test)
        decoder_train_pre = decoder(encoder(x_train))
        decoder_test_pre = decoder(encoder(x_test))
        sio.savemat(result_save_path.encoder_train_pre, {'encoder_train_result': encoder_train_pre.numpy()})
        sio.savemat(result_save_path.encoder_test_pre, {'encoder_test_result': encoder_test_pre.numpy()})
        ...
        tf.saved_model.save(mapping_model, model_save_path.mapping_model_save_path)
